#ifndef _ZERO_PROFIT_OUTPUT_H_
#define _ZERO_PROFIT_OUTPUT_H_
#if defined(_MSC_VER)
#pragma once
#endif
 ...
 */
class ZeroProfitOutput: public SecondaryOutput
{
public:
    ZeroProfitOutput();
    ...
    virtual ~ZeroProfitOutput();

    virtual ZeroProfitOutput* clone() const;
...
protected:
    ...
    DEFINE_DATA_WITH_PARENT(
        ...
        DEFINE_VARIABLE( STATE | SIMPLE | NOT_PARSABLE, "sector-profit-rate", mActualProfitRate, Value ),

        //! Stash the sector name so that we can look up the profit rate
        DEFINE_VARIABLE( SIMPLE | NOT_PARSABLE, "sector-name", mSectorName, std::string )
    )
};

#endif // _ZERO_PROFIT_OUTPUT_H_
