...
module_energy_L161.Cstorage <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "common/iso_GCAM_regID",
             ...
             FILE = "energy/GiddenEtAl_2025_PrudentStorage_SI"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c("L161.RsrcCurves_MtC_R", "L161_Gidden2025_MtC_Totals"))
    ...
    Gidden_Cstorage_MtCO2 <-get_data(all_data, "energy/GiddenEtAl_2025_PrudentStorage_SI")
    ...
    # Build up storage potentials based on Gidden et al 2025
    L161_Gidden2025_MtC_Totals <- Gidden_Cstorage_MtCO2 %>%
      inner_join(iso_GCAM_regID %>% select(-country_name, -region_GCAM3), by = "iso") %>%  # Merge and drop specified columns
      select(-iso) %>%  # Drop 'iso' column
      group_by(storage_type, GCAM_region_ID) %>%  # Group by 'storage_type' and 'region_GCAM3'
      summarise(
        technical_potential = sum(technical_potential, na.rm = TRUE) / emissions.CONV_C_CO2,
        prudent_potential = sum(prudent_potential, na.rm = TRUE) / emissions.CONV_C_CO2,
        current_oil_and_gas_potential = sum(current_oil_and_gas_potential, na.rm = TRUE) / emissions.CONV_C_CO2 # From MtCO2 -> MtC
      ) %>%
      ungroup() %>%
      # Produce output
      add_title("Carbon storage resource volumes by GCAM region based on Gidden et al 2025") %>%
      add_units("Mt C") %>%
      add_comments("Country onshore and offshore total storage volumes aggregated to GCAM regions") %>%
      add_legacy_name("L161_Gidden2025_MtC_Totals") %>%
      add_precursors("common/iso_GCAM_regID", "energy/GiddenEtAl_2025_PrudentStorage_SI") ->
      L161_Gidden2025_MtC_Totals

    return_data(L161.RsrcCurves_MtC_R, L161_Gidden2025_MtC_Totals)
  } else {
