...
module_energy_negative_emissions_budget_xml <- function(command, ...) {
  ...
  MODULE_OUTPUTS <-
    c("XML" = "negative_emissions_budget.xml",
      "XML" = "negative_emissions_budget_min_price.xml")
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    create_xml("negative_emissions_budget_min_price.xml") %>%
      add_xml_data(L270.NegEmissBudgetMaxPrice %>%
                     mutate(min.price = 0.2), "PortfolioStdMinPrice") %>%
      add_precursors(MODULE_INPUTS) ->
      negative_emissions_budget_min_price.xml
