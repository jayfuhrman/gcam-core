...
module_energy_dac_xml <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    dac_ssp1.xml <- dac_ssp2.xml <- dac_ssp3.xml <- dac_ssp4.xml <- dac_ssp5.xml <- dac_ssp1_newcosts.xml <-
      dac_ssp2_newcosts.xml <- dac_ssp3_newcosts.xml <- dac_ssp4_newcosts.xml <- dac_ssp5_newcosts.xml <- NULL
