...
module_energy_transportation_cwf_xml <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    for (i in c("CWF_med", "CWF_low", "CWF_high", "CWF_ref_shrwts")){
      ...
      if (grepl("CWF", i)) {
        ...
        L254.GlobalTranTechInterp_SSP %>%
          group_by(sector.name,subsector.name,tranTechnology) %>%
          mutate(is_first = row_number()==1) %>%
          ungroup() -> L254.GlobalTranTechInterp_SSP

        L254.DeleteGlobalTranTechInterp_SSP <-  L254.GlobalTranTechInterp_SSP %>%
          filter(is_first)
      ...
      create_xml(xml_name) %>%
        ...
        # clear out interp rules for the technologies we want to phase out to make sure the ones we want are implemented properly
    ...
    L254.GlobalTranTechInterp_SSP <- L254.GlobalTranTechInterp_cwf %>% filter(sce=='CWF_high')
    ...
    L254.GlobalTranTechShrwt_SSP <- L254.GlobalTranTechShrwt_cwf %>%  filter(sce=='CWF_high')
