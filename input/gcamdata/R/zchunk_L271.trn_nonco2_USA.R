...
module_gcamusa_L271.trn_nonco2_USA <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    UCD_techs_revised <- get_data(all_data, "energy/mappings/UCD_techs_revised")
    state_census_region <- get_data(all_data, "gcam-usa/states_subregions") %>%
    ...
    ## Update namings to reflect new GCAM names
    MARKAL_UCD_class %>%
      left_join(UCD_techs_revised %>% select(size.class,tranSubsector), by=c("UCD_class"="size.class")) %>%
      na.omit() %>%
      unique()->
      MARKAL_UCD_class
    ...
    L254.GlobalTranTechSCurve %>%
      ...
      ## Inefficient fix to allow for spreading when tibble creates numerous NAs as
      ## steepness and halflife of 2015 differ from future values
